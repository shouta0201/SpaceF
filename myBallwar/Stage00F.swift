...
class Stage00F: SKScene, SKPhysicsContactDelegate, AVAudioPlayerDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        gsCore.stageSuu = "F"      //ステージ数
        if appDelegate.gameMode == "normal" {
            gsCore.sksName = "Stage00F_normal.sks"
        }else {
            gsCore.sksName = "Stage00F.sks"
        }
        gsCore.goalPotision = -6000  //ゴールポジション　-6000
        gsCore.playerBodyCircle = 6  //プレイヤーあたり判定半径
        gsCore.scrollBool = false   //スクロールOnOff
        gsCore.scrollSpeed = 100    //スクロールスピード. ex 20000ピクセル 600秒でスロー
        gsCore.fireBool = false   //fireパーティクルOnOff
        gsCore.hCount = 20		//縦の画面数  1は667ピクセル
        gsCore.wCount = 4		//幅の画面数  1は375ピクセル
        ...
        // 再生する音源のURLを生成
        if appDelegate.audiomusic != "Fstage5" {
            appDelegate.audiomusic = "Fstage5"
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if touchNode.name == "nextStageRect"
        {
            ...
            let scene = EndingScene(size: CGSize(width: frame.size.width,height: frame.size.height))
            scene.scaleMode = .aspectFill
            let transition = SKTransition.fade(withDuration: 1.0)
            self.view!.presentScene(scene, transition:transition)
        }
