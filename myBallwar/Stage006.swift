...
class Stage006: SKScene, SKPhysicsContactDelegate, AVAudioPlayerDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        gsCore.stageSuu = "6"      //ステージ数
        if appDelegate.gameMode == "normal" {
            gsCore.sksName = "Stage006_normal.sks"
        }else {
            gsCore.sksName = "Stage006.sks"
        }
        gsCore.goalPotision = -5000  //ゴールポジション　-5000
        gsCore.playerBodyCircle = 6  //プレイヤーあたり判定半径
        gsCore.scrollBool = true   //スクロールOnOff
        gsCore.scrollSpeed = 400    //スクロールスピード. ex 20000ピクセル 600秒でスロー
        ...
        // 再生する音源のURLを生成
        if appDelegate.audiomusic != "Fstage1" {
            appDelegate.audiomusic = "Fstage1"
