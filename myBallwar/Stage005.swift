...
class Stage005: SKScene, SKPhysicsContactDelegate, AVAudioPlayerDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        gsCore.stageSuu = "5"      //ステージ数
        if appDelegate.gameMode == "normal" {
            gsCore.sksName = "Stage005_normal.sks"
        }else {
            gsCore.sksName = "Stage005.sks"
        }
        gsCore.goalPotision = -4500  //ゴールポジション　-4500
        gsCore.playerBodyCircle = 6  //プレイヤーあたり判定半径
        gsCore.scrollBool = false   //スクロールOnOff
        gsCore.scrollSpeed = 200    //スクロールスピード. ex 10000ピクセル 300秒でスロー
        gsCore.fireBool = false   //fireパーティクルOnOff
        gsCore.hCount = 10		//縦の画面数  1は667ピクセル
        gsCore.wCount = 1		//幅の画面数  1は375ピクセル
